...
const deleteDataSubmit = async (id) => {
  try {
    const response = await $api("/admin/saldo/kategori/" + id, {
      method: 'DELETE',
    });
...
onMounted(() => {
  document.title = 'Kategori Saldo - SIMKEU'
})
  ...
  <div>
    ...
    <VCard>
      <VCardItem class="pb-4">
        <VCardTitle>Kategori Saldo</VCardTitle>
      </VCardItem>
      ...
      <VCardText class="d-flex flex-wrap gap-4">
        ...
        <div class="d-flex gap-x-4 align-center">
          ...
          <VBtn color="primary" prepend-icon="ri-add-line" @click="$router.push('/admin/saldo/kategori/add')">
            Add Data
          </VBtn>
        </div>
      </VCardText>
            ...
            <VMenu activator="parent">
              <VList>
                <VListItem value="edit" prepend-icon="ri-edit-box-line"
                  @click="$router.push(`/admin/saldo/kategori/edit/${item.id}`)">
                  Edit
                </VListItem>
