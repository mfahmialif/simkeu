...
const returnPath = computed(() => {
  ...
  if (
    typeof path === "string"
    && path.startsWith("/admin/pengeluaran/umum")
  ) {
    return path
  }

  return "/admin/pengeluaran/umum"
})
...
const rowTotal = row => Math.round(Number(row.nominal || 0))
...
const validateRows = () => {
  ...
  for (let index = 0; index < rows.value.length; index += 1) {
    ...
    if (!row.nama_kegiatan) return `${label}: nama pengeluaran harus diisi.`
...
const submitBatchEdit = async () => {
  ...
  return $api("/admin/pengeluaran/umum/batch-update", {
    method: "POST",
    body: formData,
  })
}
...
const submit = async () => {
  ...
  try {
    ...
    if (isBatchEdit.value) {
      ...
      response = await $api("/admin/pengeluaran/umum/batch-store", {
        method: "POST",
        body: formData,
      })
    }
...
onMounted(() => {
  document.title = `${isBatchEdit.value ? "Edit" : "Tambah"} Pengeluaran Umum - SIMKEU`
...
<template>
  ...
  >
    ...
    >
      <VCard class="mb-5">
        <VCardItem>
          <VCardTitle>{{ isBatchEdit ? "Edit" : "Tambah" }} Pengeluaran Umum</VCardTitle>
          <VCardSubtitle>
