...
const previewSortBy = ref({ key: "name", order: "asc" })
...
const fetchAbsensiPreview = async () => {
  try {
    isLoadingPreview.value = true

    const response = await $api("/admin/pegawai/staff-absensi/preview", {
      method: "GET",
      body: {
...
const toggleSelectAllFiltered = async value => {
  ...
  try {
    ...
    const response = await $api("/admin/pegawai/staff-absensi/ids", {
      method: "GET",
      body: absensiFilterBody(),
    })
...
const openSyncDialog = () => {
  ...
  previewSearch.value = ""
  previewSortBy.value = { key: "name", order: "asc" }
  previewInitialLoading.value = true
  isSyncDialogVisible.value = true
  fetchAbsensiPreview()
}

const syncSelectedStaff = async () => {
  ...
  try {
    ...
    for (const [index, batchIds] of batches.entries()) {
      ...
      const response = await $api("/admin/pegawai/sync-staff-absensi", {
        method: "POST",
        body: {
          ids: batchIds,
      ...
      summary.failedFetches += Number(data["failed_fetches"] || 0)
      ...
      syncProgress.value.failedFetches = summary.failedFetches
    ...
    tableKey.value += 1
    showSnackbar({
      text: `Sinkronisasi staff Web Absensi selesai. ${summary.synced} data diproses.`,
      color: "success",
    })
    fetchAbsensiPreview()
  } catch (err) {
    ...
    >
      <VCard>
        <VCardItem class="pb-4">
          <VCardTitle>Sinkronisasi Staff Web Absensi</VCardTitle>
        </VCardItem>
        ...
        <VCardText>
          <div class="staff-sync-toolbar d-flex flex-wrap align-center mb-4">
            <VTextField
              v-model="previewSearch"
              class="staff-sync-search"
          ...
          >
            <div class="font-weight-medium mb-1">
              Jika data sudah ada, sinkronisasi akan memperbarui 11 kolom pegawai dan 1 kolom staff.
            </div>
            <div class="text-body-2">
              Acuan update: <strong>kode pegawai = ID user Web Absensi</strong>. Kolom pegawai yang diperbarui:
              nama, jenis_kelamin, tipe, tempat_lahir, tanggal_lahir, alamat, email, hp, nomer_rekening, bank, status.
              Kolom staff yang diperbarui: jabatan. Data dengan kode yang sudah dipakai tipe selain staff akan dilewati.
            </div>
          </VAlert>
          ...
          <div
            ...
            class="staff-sync-progress mb-4"
          >
            ...
            <div class="d-flex flex-wrap gap-4 text-caption text-medium-emphasis mt-2">
              ...
              <span>Dilewati {{ syncProgress.skipped + syncProgress.failedFetches }}</span>
            </div>
          </div>
          ...
          >
            ...
            >
              <div class="text-center pa-4">
                <VProgressCircular
                  ...
                  class="mb-2"
                />
                <div>Memuat data staff Web Absensi...</div>
              </div>
            </template>
            ...
            >
              <div class="text-center pa-4">
                Tidak ada data staff Web Absensi.
              </div>
            </template>
        ...
        <VCardText class="d-flex justify-end flex-wrap gap-4">
          <div
            v-if="selectedCount"
            class="staff-sync-selected text-medium-emphasis"
          >
