...
const props = defineProps({
  ...
  returnPath: {
    type: String,
    default: "/admin/pengeluaran/umum",
  },
  initialRekapId: {
...
const title = ref("Tambah Pengeluaran Umum")
...
const total = computed(() => Math.round(Number(nominal.value || 0)))
...
const onSubmit = async () => {
  ...
  const url = editId
    ? `/admin/pengeluaran/umum/${editId}`
    : "/admin/pengeluaran/umum"
...
watch(
  ...
  () => {
    if (props.typeForm === "edit" && props.dataForm) {
      title.value = `Update Pengeluaran Umum: ${props.dataForm.nama_kegiatan || props.dataForm.tanggal || ""}`
