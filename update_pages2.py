        ...
        if file.endswith('.vue'):
            ...
            for field in fields_to_update:
                ...
                def replacer1(m):
                    ...
                    res = base + 'hide-details="auto"' + m.group(2)
                    if 'formatRupiah' not in m.group(0):
                        res = res.replace('hide-details="auto"', 'hide-details="auto" :hint="formatRupiah(' + field + ' || 0)" persistent-hint')
                    return res
                ...
                def replacer2(m):
                    if 'hide-details' in m.group(0):
                        return m.group(0) # handled by pattern 1
                    base = m.group(1).replace('<LazyTextField', '<VTextField')
                    res = base + ' hide-details="auto"' + m.group(2)
                    if 'formatRupiah' not in m.group(0):
                        res = res.replace('hide-details="auto"', 'hide-details="auto" :hint="formatRupiah(' + field + ' || 0)" persistent-hint')
